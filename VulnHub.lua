...
task.spawn(function()
    pcall(function()
        ...
        pcall(function()
            local successPages, pages = pcall(function()
                return Players:GetFriendsAsync(_v12.UserId)
            end)
            
            if successPages and pages then
                while true do
                    for _, friend in ipairs(pages:GetCurrentPage()) do
                        local successStatus, isOnline = pcall(function()
                            return friend.IsOnline and friend.PlaceId == game.PlaceId and friend.GameId == game.GameId
                        end)
                        
                        if successStatus and isOnline then
                            table.insert(FriendsInServer, friend.Username .. " (" .. friend.DisplayName .. ")")
                        end
                    end
...
local _vNotifSound = Instance.new("Sound")
_vNotifSound.Name = "VulnSec_NotifSound"
_vNotifSound.SoundId = "rbxassetid://130467592920597"
_vNotifSound.Volume = 1
_vNotifSound.Parent = _v9

local _vExecSound = Instance.new("Sound")
_vExecSound.Name = "VulnSec_ExecSound"
_vExecSound.SoundId = "rbxassetid://119827553755016"
_vExecSound.Volume = 1
_vExecSound.Parent = _v9
...
local _vAfkEnabled = false
_f1(_v12.Idled:Connect(function()
    if _vAfkEnabled and _v13 then
        local _vVirtualUser = game:GetService("VirtualUser")
        _vVirtualUser:CaptureController()
        _vVirtualUser:ClickButton2(Vector2.new())
    end
end))

_f7("Enable Anti-AFK", false, _v61, function(_p1)
    _vAfkEnabled = _p1
    if _p1 then
        _f3("Anti-AFK Enabled", Color3.fromRGB(0, 255, 200))
    else
        _f3("Anti-AFK Disabled", Color3.fromRGB(255, 180, 50))
    end
end)

_f6("Auto Leave on Target", _v61)

local _vAutoLeaveEnabled = false
local _vAutoLeaveTarget = ""

local _vAutoLeaveBox = Instance.new("TextBox")
_vAutoLeaveBox.Size = UDim2.new(1, -8, 0, 32)
_vAutoLeaveBox.BackgroundColor3 = Color3.fromRGB(20, 20, 24)
_vAutoLeaveBox.PlaceholderText = "Enter target username..."
_vAutoLeaveBox.PlaceholderColor3 = Color3.fromRGB(100, 100, 105)
_vAutoLeaveBox.Text = ""
_vAutoLeaveBox.TextColor3 = Color3.fromRGB(255, 255, 255)
_vAutoLeaveBox.Font = Enum.Font.GothamMedium
_vAutoLeaveBox.TextSize = 11
_vAutoLeaveBox.TextXAlignment = Enum.TextXAlignment.Left
_vAutoLeaveBox.Parent = _v61

Instance.new("UICorner", _vAutoLeaveBox).CornerRadius = UDim.new(0, 6)
local _vAutoLeaveBoxStroke = Instance.new("UIStroke", _vAutoLeaveBox)
_vAutoLeaveBoxStroke.Color = Color3.fromRGB(28, 28, 32)
_vAutoLeaveBoxStroke.Thickness = 1

_f1(_vAutoLeaveBox.Focused:Connect(function()
    _v2:Create(_vAutoLeaveBoxStroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(0, 180, 255)}):Play()
end))

_f1(_vAutoLeaveBox.FocusLost:Connect(function()
    _v2:Create(_vAutoLeaveBoxStroke, TweenInfo.new(0.15), {Color = Color3.fromRGB(28, 28, 32)}):Play()
    _vAutoLeaveTarget = _vAutoLeaveBox.Text:gsub("%s+", "")
end))

local function _checkAndKickTarget(_player)
    if not _vAutoLeaveEnabled or _vAutoLeaveTarget == "" then return end
    if _player.Name:lower() == _vAutoLeaveTarget:lower() or _player.DisplayName:lower() == _vAutoLeaveTarget:lower() then
        _v12:Kick("[VulnHub - Auto Leave] Target user joined your current server")
    end
end

_f7("Enable Auto Leave", false, _v61, function(_p1)
    _vAutoLeaveEnabled = _p1
    _vAutoLeaveTarget = _vAutoLeaveBox.Text:gsub("%s+", "")
    if _p1 then
        if _vAutoLeaveTarget == "" then
            _f3("Please enter a target username first!", Color3.fromRGB(255, 80, 80))
        else
            _f3("Auto Leave Enabled for: " .. _vAutoLeaveTarget, Color3.fromRGB(0, 255, 200))
            for _, player in ipairs(_v1:GetPlayers()) do
                _checkAndKickTarget(player)
            end
        end
    else
        _f3("Auto Leave Disabled", Color3.fromRGB(255, 180, 50))
    end
end)

_f1(_v1.PlayerAdded:Connect(function(player)
    _checkAndKickTarget(player)
end))
...
for _v267 in _v266:gmatch("([^\r\n]*)\r?\n?") do
    ...
    local _tag, _content = _v267:match("^%s*%[(.-)%]%s*(.*)")
    
    if _v267 == "" then
        _v268.Size = UDim2.new(1, -10, 0, 6)
        _v268.Text = ""
    elseif _tag then
        ...
        if _tag == "HEADER" then
            ...
            _v268.Font = Enum.Font.GothamBold
            _v268.TextSize = 13
        elseif _tag == "SECTION" then
            _v268.Text = _content
            _v268.TextColor3 = Color3.fromRGB(255, 255, 255)
            _v268.Font = Enum.Font.GothamBold
            _v268.TextSize = 12
        elseif _tag == "BULLET" then
            _v268.Text = "  • " .. _content
            _v268.TextColor3 = Color3.fromRGB(200, 200, 200)
            _v268.Font = Enum.Font.GothamMedium
            _v268.TextSize = 11
        elseif _tag == "INDENT" then
            _v268.Text = "      - " .. _content
            _v268.TextColor3 = Color3.fromRGB(150, 150, 150)
            _v268.Font = Enum.Font.Gotham
            _v268.TextSize = 11
        else
            _v268.Text = _content
            _v268.TextColor3 = Color3.fromRGB(180, 180, 180)
            _v268.Font = Enum.Font.GothamMedium
            _v268.TextSize = 11
        end
    else
